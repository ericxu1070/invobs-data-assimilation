# DIGEST: PyTorch_InvObs_DA_v2_PaperFaithful.ipynb  (slug: L96_PaperFaithful)

## [md cell 0]
# PyTorch InvObs DA — v2 Paper-Faithful
...
# Bootstrap: clone the repo into Colab and wire up sys.path so imports work.
if 'google.colab' in sys.modules:
    _REPO = '/content/invobs-data-assimilation'
    if not os.path.isdir(_REPO):
        subprocess.check_call([
            'git', 'clone', '--depth=1',
            'https://github.com/ericxu1070/invobs-data-assimilation.git',
            _REPO,
        ])
    os.chdir(_REPO)
...
```
--- output ---
L96: grid=40, observe_every=4, outer_dt=0.1
...
## [code cell 5]
```python
# Training-set parameters from INSTRUCTIONS_FROM_REVIEW.md (Phase 1).
N_TRAIN     = 32000
T_TRAIN     = 20
N_WARMUP    = 1000
N_EPOCHS    = 500
...
```
--- output ---
  [cache] loaded l96_train_data_n32000_T20_warmup1000_sigma0.0.pt
Loaded training set: X=(32000, 20, 40), Y=(32000, 20, 10)

## [code cell 6]
```python
def train_inverter_v2(model, X, Y, n_epochs=N_EPOCHS, batch_size=BATCH_SIZE, lr=LR,
...
## [code cell 8]
```python
# --- Train the original PyTorch port (residual stack with GELU + bilinear upsample). ---
...
```
--- output ---
  [cache] loaded l96_inverter_PORT_sigma0.0_n32000_ep500.pt
Loaded PORT from cache: 500 epochs, final MSE = 0.3714

## [code cell 9]
```python
# --- Train the paper-faithful port (SiLU + cubic upsample + BN, no skips). ---
...
```
--- output ---
  [cache] loaded l96_inverter_PAPER_sigma0.0_n32000_ep500.pt
Loaded PAPER from cache: 500 epochs, final MSE = 0.4924

## [code cell 10]
```python
# --- Side-by-side training curves on the same axis. ---
...
>>> FIGURE EMBEDDED: figures/L96_PaperFaithful__cell10__out00.png
...
```
--- output ---
Eval shapes: X=(100, 20, 40), Y=(100, 20, 10)
  [PORT ] test L1 = 0.3838   x0 RMSE = 0.9973 +/- 0.3261
  [PAPER] test L1 = 0.4470   x0 RMSE = 1.1635 +/- 0.4036

PAPER/PORT test L1 ratio = 1.165  (WITHIN 30% of port)

## [code cell 13]
```python
# Visual sanity: per-sample x0 reconstruction for the first 4 trajectories.
...
>>> FIGURE EMBEDDED: figures/L96_PaperFaithful__cell13__out00.png
...
```
--- output ---
  [cache] loaded l96_x0_mean.pt
X0_mean: shape=(40,), mean=2.3448, std=0.0944

## [code cell 17]
```python
# --- Acceptance check: direct init RMSE comparison at sigma_obs=0, T=10. ---
T_PHASE2     = 10
N_EVAL_BASE  = 100
...
```
--- output ---
sigma_obs=0, T=10, N=100 trajectories — direct init x0 RMSE:
  repeat-baseline: 4.7828 +/- 0.5006
  paper-baseline:  3.0892 +/- 0.2539
  paper/repeat ratio = 0.646  (PASS: paper noticeably lower)

## [code cell 18]
```python
# --- Noise-sweep bar chart of 4D-Var endpoints (sigma_b=1.0 fixed). ---
...
C: shape=(40, 40), cond=8.72
  [cache] loaded l96_phase2_baseline_sweep_N100_T10.pt
Loaded Phase 2 results from cache.

## [code cell 19]
```python
# --- Bar chart: panel per noise level, baseline x mode bars. ---
...
>>> FIGURE EMBEDDED: figures/L96_PaperFaithful__cell19__out00.png
...
## [code cell 20]
```python
# --- Hovmöller comparison at sigma=0.5: truth vs each baseline + obs. ---
sigma_show = 0.5
T_HOV = 30
sample_idx = 0
...
>>> FIGURE EMBEDDED: figures/L96_PaperFaithful__cell20__out00.png
...
```
--- output ---
sigma_b grid:    [0.1, 0.3, 1.0, 3.0, 10.0]
...
## [code cell 24]
```python
# --- Train PAPER inverter at each noise level in NOISE_LEVELS_PHASE4. ---
...
```
--- output ---
  [cache] loaded l96_train_data_n32000_T20_warmup1000_sigma0.1.pt
  [cache] loaded l96_inverter_PAPER_sigma0.1_n32000_ep500.pt
  [paper sigma=0.1] loaded from cache (final MSE = 0.6321)
  [cache] loaded l96_train_data_n32000_T20_warmup1000_sigma0.5.pt
  [cache] loaded l96_inverter_PAPER_sigma0.5_n32000_ep500.pt
  [paper sigma=0.5] loaded from cache (final MSE = 1.3338)
  [cache] loaded l96_train_data_n32000_T20_warmup1000_sigma1.0.pt
  [cache] loaded l96_inverter_PAPER_sigma1.0_n32000_ep500.pt
  [paper sigma=1.0] loaded from cache (final MSE = 2.2201)

Ready: 3 PAPER inverters across noise levels.

## [code cell 25]
```python
# --- Sweep sigma_b for every (init x mode x noise) combo. ---
...
```
--- output ---
  [cache] loaded l96_phase4_sigmab_sweep_N16_T10_sigmab0.1-0.3-1.0-3.0-10.0.pt
Loaded Phase 4 sweep results from cache.

## [code cell 26]
```python
# --- Tabulate the optimal sigma_b per cell. ---
import pandas as pd
rows = []
for sigma_obs in NOISE_LEVELS_PHASE4:
...
```
--- output ---
 sigma_obs           combo  best_sigma_b  best_RMSE  untuned_RMSE_at_sigma_b=1
       0.1     paper + obs           0.1   2.465790                   2.594196
       0.1  paper + hybrid           3.0   0.621480                   0.623843
       0.1    invobs + obs           0.3   0.882598                   0.890846
       0.1 invobs + hybrid          10.0   0.619812                   0.621928
       0.5     paper + obs           0.3   2.392565                   2.493539
       0.5  paper + hybrid           1.0   1.223034                   1.223034
       0.5    invobs + obs           0.3   1.550101                   1.727582
       0.5 invobs + hybrid           1.0   1.168052                   1.168052
       1.0     paper + obs           0.3   2.707974                   2.740130
       1.0  paper + hybrid           1.0   1.846222                   1.846222
       1.0    invobs + obs           1.0   1.867065                   1.867065
       1.0 invobs + hybrid           1.0   1.675684                   1.675684

## [code cell 27]
```python
# --- Acceptance check: optimal sigma_b for invobs+hybrid < for paper+obs (per noise). ---
...
Acceptance check (per noise level): is invobs+hybrid optimal sigma_b < paper+obs optimal sigma_b?
  sigma_obs=0.1: invobs+hybrid sigma_b=10.00, paper+obs sigma_b= 0.10   -> FAIL
  sigma_obs=0.5: invobs+hybrid sigma_b= 1.00, paper+obs sigma_b= 0.30   -> FAIL
  sigma_obs=1.0: invobs+hybrid sigma_b= 1.00, paper+obs sigma_b= 0.30   -> FAIL

Tuned <= untuned (sigma_b = 1.0)?
  sigma_obs=0.1, paper + obs       : tuned=2.4658, untuned=2.5942, gain=0.1284
  sigma_obs=0.1, paper + hybrid    : tuned=0.6215, untuned=0.6238, gain=0.0024
  sigma_obs=0.1, invobs + obs      : tuned=0.8826, untuned=0.8908, gain=0.0082
  sigma_obs=0.1, invobs + hybrid   : tuned=0.6198, untuned=0.6219, gain=0.0021
  sigma_obs=0.5, paper + obs       : tuned=2.3926, untuned=2.4935, gain=0.1010
  sigma_obs=0.5, paper + hybrid    : tuned=1.2230, untuned=1.2230, gain=0.0000
  sigma_obs=0.5, invobs + obs      : tuned=1.5501, untuned=1.7276, gain=0.1775
  sigma_obs=0.5, invobs + hybrid   : tuned=1.1681, untuned=1.1681, gain=0.0000
  sigma_obs=1.0, paper + obs       : tuned=2.7080, untuned=2.7401, gain=0.0322
  sigma_obs=1.0, paper + hybrid    : tuned=1.8462, untuned=1.8462, gain=0.0000
  sigma_obs=1.0, invobs + obs      : tuned=1.8671, untuned=1.8671, gain=0.0000
  sigma_obs=1.0, invobs + hybrid   : tuned=1.6757, untuned=1.6757, gain=0.0000

## [code cell 28]
```python
# --- Tuned bar chart: panel per noise level, bars = combos, sigma_b annotated. ---
...
>>> FIGURE EMBEDDED: figures/L96_PaperFaithful__cell28__out00.png
