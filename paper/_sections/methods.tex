...
The first testbed is the 1D Lorenz-96 model, a periodic chain of $K=40$ grid points
...
(RK4) scheme in which each outer step $\dtout = 0.1$ MTU is
composed of ten inner RK4 substeps of size $\dtin = 0.01$. The observation operator
...
(dopri5) solver. Starting from identical attractor
...
warmup diagnostic confirms the flow reaches an approximately quasi-stationary
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.68\linewidth]{noise_distributions.png}
...
(SiLU) activations, batch normalization (BN) after
every convolution, cubic periodic upsampling, and a
...
many-to-one operator $\Hop$ from inside the objective, and $\sigma_p$ is the
physics-space error scale (set to $\sigma_p = 0.1$ in our runs). The \emph{hybrid}
...
using a strong-Wolfe line search. The entire
...
observation-space iterations (for the single-window Lorenz-96 runs, $200$ then
$300$; for Lorenz-96 cycling, $100$ then $400$), whereas pure observation-space
runs spend the full iteration budget on $\Jb+\Jo$.
...
cycling trusts the propagated analysis, it uses a tighter background-error
standard deviation ($\sigb=0.3$) than the cold-start single window ($\sigb=1.0$).
