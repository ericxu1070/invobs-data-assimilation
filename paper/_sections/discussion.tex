...
\texttt{torch.fft.rfft2}, which ran on CPU only in our stack, so all 2D
experiments were validation-scale: a tiny ensemble, a
...
invobs$+$hybrid to prefer a \emph{smaller} optimal background-error standard
deviation $\sigb$ than the paper baseline---reasoning that the learned lift
supplies a stronger physics-space prior, so the analysis could lean on the
background more (a smaller $\sigb$ raises the background weight
$1/(2\sigb^2)$). This did not hold: invobs$+$hybrid selected
...
background-error standard deviation, a short integrator-limited Lyapunov time, and
