...
Our PyTorch \citep{paszke2019pytorch} port integrates Lorenz-96 (grid size $40$,
forcing $F=8$) with a fixed-step RK4 scheme, ten inner
substeps of $\Delta t = 0.01$ per outer step $dt = 0.1$, whereas the reference paper
uses an adaptive dopri5 solver \citep{dormand1980family}. To check
...
(Figure~\ref{fig:l96-integrator}). Because the per-step RK4 truncation error is
$O(\Delta t^4) = O(10^{-8})$, well below the adaptive tolerances, the two integrators
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.7\linewidth]{L96_Integrator__cell08__out00.png}
  \caption{Fixed-step RK4 versus adaptive dopri5 integrator divergence
  on Lorenz-96. The vertical axis (log scale) is the $L^2$ state difference
  $\lVert \xstate_{\mathrm{RK4}}(t)-\xstate_{\mathrm{adapt}}(t)\rVert_2$ and the
  horizontal axis is model time $t$ in MTU. The solid blue curve is the median over five
  warmed-up attractor states and the shaded band is the min/max envelope; the dashed grey
  line marks the order-one threshold. From identical initial conditions the two
  integrators agree to near machine precision while $t \ll T_L$, then the per-step
  truncation difference is amplified at the chaotic rate, crossing the order-one scale at
  a median $t^\ast = 7.70$ MTU (range $[6.50, 8.30]$; $6.30$ MTU for the single trajectory
  shown in the first panel) and saturating near $28.5$ once the trajectories fully
  decorrelate. The takeaway is that $t^\ast$ is the operational ceiling on
  assimilation-window length over which the two simulators represent the same physics; our
  single-window experiments use $T = 1.0$ MTU, comfortably inside it.}
  \label{fig:l96-integrator}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.7\linewidth]{L96_Integrator__cell10__out00.png}
  ...
  after which the separation saturates near the attractor diameter. The measured
  $T_L = 0.442$ MTU is somewhat shorter than the paper's expected $\sim\!0.6$ MTU, a known
  consequence of the fixed-step RK4 integrator being slightly too coarse to resolve the
  fastest unstable directions; it sets the chaotic divergence rate seen in
  Figure~\ref{fig:l96-integrator}.}
...
(SiLU activations \citep{elfwing2018sigmoid}; channel pyramid
$128\!\to\!64\!\to\!32\!\to\!16$; BN after every convolution
\citep{ioffe2015batch}; cubic-periodic upsampling; no skip connections) reaches $0.4924$
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.7\linewidth]{L96_PaperFaithful__cell10__out00.png}
  ...
  identical Adam settings ($500$ epochs); the vertical axis is training MSE (log scale)
  and the horizontal axis is epoch. The original PyTorch port (blue; GELU activations,
  flat width-$32$ residual stack, bilinear upsampling) converges to final MSE $0.3714$,
  while the paper-faithful inverter (orange; SiLU activations,
  $128\!\to\!64\!\to\!32\!\to\!16$ channels, BN after every convolution, cubic-periodic
  upsampling, no skip connections) converges to $0.4924$. The two curves track closely
  throughout, the port settling slightly lower. Despite substantial architectural
  differences, the paper-faithful inverter reproduces the port's reconstruction quality:
  on a held-out $100$-trajectory set their test $L_1$ losses are $0.3838$ versus $0.4470$
  (ratio $1.165$, within the $30\%$ acceptance band).}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\linewidth]{L96_PaperFaithful__cell19__out00.png}
  ...
  optimization (grey) is worst at every noise level; replacing it with the
  climatological-mean baseline and, especially, adding hybrid optimization lowers RMSE
  substantially (the climatological+hybrid bars are lowest, most clearly at
  $\sigobs = 0.1$). A better cold-start prior and hybrid optimization each reduce analysis
  error even before the background weight $\sigb$ is tuned, motivating the per-cell
  $\sigb$ sweep that follows. (Direct $\xstate_0$ RMSE of the raw initializers at
  $\sigobs = 0$ is $4.78$ for repeat vs.\ $3.09$ for climatological mean, ratio $0.646$.)}
  \label{fig:l96-baselinebar}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{L96_PaperFaithful__cell20__out00.png}
...
Our central single-window experiment sweeps the background weight
$\sigb \in \{0.1, 0.3, 1.0, 3.0, 10.0\}$ for every (initializer $\times$
...
4D-Var setting with observation noise. Second, hybrid optimization dramatically beats
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.97\linewidth]{L96_PaperFaithful__cell28__out00.png}
  ...
  values reveal an honest negative result: the expected ordering ``invobs$+$hybrid prefers
  a smaller $\sigb$ than paper$+$obs'' does not hold (invobs$+$hybrid selects
  $\sigb = 10.0/1.0/1.0$ versus paper$+$obs $0.1/0.3/0.3$), and tuning $\sigb$ helps only
  marginally over the untuned $\sigb = 1$ in most cells.}
...
$\sigobs \in \{0.0, 0.5\}$. Single-window (cold-start) runs use background variance
$\sigb = 1.0$, while cycling runs use a tighter $\sigb = 0.3$ because the cycled
background---the previous analysis propagated forward to the new window start---is
trusted more than a cold start. Optimization is L-BFGS \citep{liu1989limited} in
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{L96_SlidingWindow__cell27__out00.png}
  ...
  optimization-capable methods reach analysis $\rmse \approx 1$ at $\sigobs = 0$ and
  $\approx 1.6$--$1.9$ at $\sigobs = 0.5$, while baseline$+$obs-only stays near $5$.
  Physics-space (hybrid) warm-starting is essential to rescue the climatological-mean
  baseline from a poor observation-space local minimum; the best single-window choice is
  baseline$+$hybrid at $\sigobs = 0$ and invobs$+$hybrid at $\sigobs = 0.5$, though their
  margin at $\sigobs = 0$ is within ensemble scatter.}
  \label{fig:l96-swA}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.92\linewidth]{L96_SlidingWindow__cell30__out00.png}
  ...
  on Lorenz-96 cycling skill---differences are confined to the window and shortest lead
  times---so the best stride is a narrow tie-break rather than a strong preference.}
  \label{fig:l96-swB1}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\linewidth]{L96_SlidingWindow__cell31__out00.png}
  ...
  over-determines the $10$ observed components and few cycles leave $N = 50$ sampling noise
  dominant. Cycling demonstrably reduces background error into a better analysis each
  cycle, with the accumulated-history benefit largest when observations are noisy.}
  \label{fig:l96-swB2}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.97\linewidth]{L96_SlidingWindow__cell34__out00.png}
  ...
  at $\sigobs = 0.5$ with long history $T_\mathrm{obs} = 48$, and marginal (curves overlap
  within $N = 50$ scatter) at $\sigobs = 0$, where one well-posed $8$-step window already
  pins the observed state. Deep in the chaotic forecast all methods converge to the same
  climatological saturation: cycling improves the analysis and short-lead forecast, not the
  Lyapunov-time-limited long-range predictability.}
  \label{fig:l96-swC}
\end{figure}

\paragraph{Experiment E (Hovm\"oller diagnostics).}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.97\linewidth]{L96_SlidingWindow__cell36__out00.png}
  \caption{Experiment E: space-time (Hovm\"oller) diagnostics for one ensemble member
  ...
  the window; the signed-error rows are near-white inside and immediately after the window
  and develop structured stripes as the chaotic forecast decorrelates, with SW-best showing
  visibly cleaner near-window error than the cold single-window invobs$+$hybrid at
  $\sigobs = 0.5$. Forecast error growth far from the window is comparable across methods;
  this is a single illustrative member ($N = 50$ overall), not a statistical summary.}
  \label{fig:l96-swE}
\end{figure}
